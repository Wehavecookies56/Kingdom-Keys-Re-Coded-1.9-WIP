package uk.co.wehavecookies56.common.container;
...
import uk.co.wehavecookies56.common.container.slot.SlotSynthesisBag;
...
public class ContainerSynthesisBagS extends Container {
	...
	public ContainerSynthesisBagS (EntityPlayer player, InventoryPlayer invPlayer, InventorySynthesisBagS invBag) {
		...
		for (i = 0; i < 3; ++i)
			for (j = 0; j < 9; ++j)
				addSlotToContainer(new Slot(invPlayer, j + i * 9 + 9, 8 + j * 18, 58 + i * 18));

		for (i = 0; i < 9; ++i)
			addSlotToContainer(new Slot(invPlayer, i, 8 + i * 18, 116));
	}
