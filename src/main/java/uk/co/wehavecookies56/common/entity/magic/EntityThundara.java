package uk.co.wehavecookies56.common.entity.magic;
...
public class EntityThundara extends Entity {
	...
	public EntityThundara (World world, EntityPlayer sender, double x, double y, double z) {
		...
		if (!list.isEmpty()) for (int i = 0; i < list.size(); i++) {
			...
			if (e instanceof EntityLivingBase) {
				...
				if (!worldObj.isRemote) PacketDispatcher.sendToAllAround(new SpawnThunderEntity(this, 2), player, 64.0D);
	...
	public void onUpdate () {
		...
		int rotation = 0;
		if (!worldObj.isRemote) PacketDispatcher.sendToAllAround(new SpawnThunderParticles(this, 2), player, 64.0D);
		double r = 1.5D;
